...
- `cache_file_path` : Chemin vers l'emplacement où sera créé le fichier JSON pour stocker le cache des documents traités. Ce fichier de cache permet de réduire de manière significative le temps de traitement lorsque le même document est traité plusieurs fois lors de futures exécutions. Si ce paramètre n'est pas fourni ou si le chemin est invalide, la fonction ne sauvegardera aucun cache, ce qui entraînera un temps de traitement plus long, car le document devra être retraité à chaque exécution.
